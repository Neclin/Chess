...
namespace Chess.Core.Search
{
    ...
    {
        public const int PieceSquareKeyCount = 12 * 64;
        public const int CastlingRightsKeyCount = 4;
        public const int EnPassantFileKeyCount = 8;
        public const int TurnKeyCount = 1;
        public const int TotalKeyCount = PieceSquareKeyCount + CastlingRightsKeyCount + EnPassantFileKeyCount + TurnKeyCount;

        public const int CastlingRightsKeyOffset = PieceSquareKeyCount;
        public const int EnPassantFileKeyOffset = PieceSquareKeyCount + CastlingRightsKeyCount;
        public const int TurnKeyOffset = PieceSquareKeyCount + CastlingRightsKeyCount + EnPassantFileKeyCount;

        public static ulong PieceSquareKey(PieceType pieceType, PieceColor pieceColor, int squareIndex)
        {
            int polyglotPieceIndex = PolyglotPieceIndex(pieceType, pieceColor);
            return PolyglotRandomConstants[polyglotPieceIndex * 64 + squareIndex];
        }

        public static ulong CastlingRightsKey(int castlingRightOffsetWithinFour)
        {
            return PolyglotRandomConstants[CastlingRightsKeyOffset + castlingRightOffsetWithinFour];
        }

        public static ulong EnPassantFileKey(int fileIndex)
        {
            return PolyglotRandomConstants[EnPassantFileKeyOffset + fileIndex];
        }

        public static ulong TurnKey()
        {
            return PolyglotRandomConstants[TurnKeyOffset];
        }

        public static int PolyglotPieceIndex(PieceType pieceType, PieceColor pieceColor)
        {
            int blackOrWhiteOffset = pieceColor == PieceColor.White ? 1 : 0;
            int pieceTypeOffset = (int)pieceType - 1;
            return 2 * pieceTypeOffset + blackOrWhiteOffset;
        }

        public static ulong Compute(BoardState board)
        {
            ulong polyglotKey = 0;
            ...
            for (int colorIndex = 0; colorIndex < 2; colorIndex++)
            {
                ...
                {
                    PieceType pieceType = (PieceType)pieceTypeIndex;
                    ...
                    while (pieceBitboard != 0)
                    {
                        ...
                        polyglotKey ^= PieceSquareKey(pieceType, pieceColor, squareIndex);
                    }
                }
            }

            if ((board.Castling & CastlingRights.WhiteKingside) != 0) polyglotKey ^= CastlingRightsKey(0);
            if ((board.Castling & CastlingRights.WhiteQueenside) != 0) polyglotKey ^= CastlingRightsKey(1);
            if ((board.Castling & CastlingRights.BlackKingside) != 0) polyglotKey ^= CastlingRightsKey(2);
            if ((board.Castling & CastlingRights.BlackQueenside) != 0) polyglotKey ^= CastlingRightsKey(3);

            if (board.EnPassantSquare >= 0 && SideToMoveHasEnPassantCapturer(board))
                polyglotKey ^= EnPassantFileKey(Square.FileIndex(board.EnPassantSquare));
        ...
        private static bool SideToMoveHasEnPassantCapturer(BoardState board)
        {
            int enPassantTargetSquare = board.EnPassantSquare;
            int enPassantFile = Square.FileIndex(enPassantTargetSquare);
            int enPassantRank = Square.RankIndex(enPassantTargetSquare);
            ulong sideToMovePawnBitboard = board.BitboardFor(PieceType.Pawn, board.SideToMove);

            int captureFromRank = board.SideToMove == PieceColor.White ? enPassantRank - 1 : enPassantRank + 1;
            if (captureFromRank < 0 || captureFromRank > 7) return false;

            if (enPassantFile > 0)
            {
                int leftCaptureFromSquare = Square.FromFileAndRank(enPassantFile - 1, captureFromRank);
                if ((sideToMovePawnBitboard & (1UL << leftCaptureFromSquare)) != 0) return true;
            }
            if (enPassantFile < 7)
            {
                int rightCaptureFromSquare = Square.FromFileAndRank(enPassantFile + 1, captureFromRank);
                if ((sideToMovePawnBitboard & (1UL << rightCaptureFromSquare)) != 0) return true;
            }
            return false;
        }

        private static readonly ulong[] PolyglotRandomConstants = new ulong[TotalKeyCount]
        {
